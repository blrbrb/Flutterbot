...
import Discord, { Guild, GuildResolvable, Snowflake } from "discord.js"
import { DataTypes, fsIDResolvable, fsGuildIDResolvable, fsUserIDResolvable, fsObject } from './types';
import { IsSnowflake, Log, getArrayType, mergeArrays, resolveGuildID, resolveID } from './utilities';
...
export  class SimpleDatabase {
    ...
    /**
     * Simple, probably-good-enough database 
     * @constructor
     * @param {string} filePath
     * @param {Function} Log
     * @param {boolean} debug_print
     */
    filePath: string; 
    debug_print: boolean;
    data: fsObject; 
...
_printAcessInfo(Key='', value:any, ID:Snowflake) {
    ...
    set(IDResolvable:fsIDResolvable, Key:string, value:any):void {
        let ID:Snowflake, current:any, exists:boolean
        ...
        if (Array.isArray(current[Key])) // the existing property is an array. 
        {
            ...
            if (Array.isArray(value)) //the new value is also an array. Merge.
            {
                ...
                try {
                    ...
                    const appended = mergeArrays(current[`${Key}`], value);
            ...
            {
                const arrayType = getArrayType(current[`${Key}`]).type;
                ...
                switch (arrayType) //make sure it's the correct datatype before pushing.
                {
                    ...
                    case DataTypes.Object:
                        if (typeof(value) === 'object') {
                            ...
                            const exists = current[Key].some((obj:object) => obj === value);
                            console.log(exists);
                            if (!exists) {
                                current[Key].push(value);
                                break;
   ...
   _updateObject(New:fsObject, Old:fsObject): fsObject {
     ...
     * @returns {void}
     */
    setGuildCoolDown(GuildResolvable:fsGuildIDResolvable, cooldowntime:number): void {
        try {
            ...
            if (guildID && this.hasGuild(guildID)) {
                ...
                if (Array.isArray(current)) {
                    if (Array.isArray(value)) //the new value is also an array. Merge.
                    {
                        ...
                        try {
                            const appended = mergeArrays(current, value);
                            current = appended;
                        } catch (error:any) {
                            this.log('red', error);
                        }
                    ...
                    {
                        const arrayType = getArrayType(current).type;
     ...
     */
    hasGuild(GuildResolvable:GuildResolvable): boolean {
        let ID = resolveGuildID(GuildResolvable);
