...
const {defaultValues, errorMessage} = require('../lang/en.js');
...
module.exports = {
    ...
    Flutterbot.DisTube.on("playSong", (queue, song) => {
        ...
        queue.textChannel.send(defaultValues.onPlaying(song));
    });
    ...
    Flutterbot.DisTube.on("empty", (queue) => { queue.textChannel.send(defaultValues.onQueueFinish())});
	
    Flutterbot.DisTube.on("addSong", (queue, song) => {
        
        queue.textChannel.send(defaultValues.onAddSong(song));
    });
